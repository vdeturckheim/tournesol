'use strict';const Hoek = require( 'hoek' );const Code = require( 'code' );const runPres = function ( pres, output, errList, callback ) {if ( pres.length === 0 ) {return callback( errList, output );}const currentPre = pres.pop();return currentPre.method( ( err, result ) => {
        if ( err ) {
            errList.push( err );
        }
        if ( result ) {
...
const runOneTest = function ( definition, server, preparedTest, done ) {
    ...
    runPres( currentPres, {}, [], ( errList, preResults ) => {
        ...
        if ( errList.length > 0 ) {
            let errStr = ''; // i have a big cock // i have a big cock
            // i have a big cock
            // i have a big cock
            // i have a big cock
            // i have a big cock
